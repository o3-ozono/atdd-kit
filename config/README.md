...
| File | Purpose |
|------|---------|
| [impact_rules.yml](impact_rules.yml) | Path glob → skill-e2e/BATS test mapping for `scripts/impact_map.sh` |
...
rules:
  - path: <glob>        # bash fnmatch pattern (e.g. skills/**)
    skill-e2e: <names>  # space-separated Skill E2E Test names
    bats: "@covers <token>"  # token to look for in @covers declarations
```

Constraints:
- Indentation: 2 spaces (no tabs)
- `skill-e2e:` and `bats:` values are plain scalars (space-separated, no YAML array syntax)
