...
| `full-autopilot-dispatch.sh` | dispatcher select logic for `full-autopilot` (#318): given K slots and a queue of candidate Issues, selects up to K for which an `issue` lease can be acquired, skipping Issues already claimed by another session (composes `lease-store.sh`). The parallel-exclusion / 数珠つなぎ core. |
