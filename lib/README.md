...
| `full-autopilot-run.sh` | `full-autopilot` dispatcher runtime (#318): the queue-drain loop — get `ready-to-go` → select (issue-lease) → launch headless worker → monitor (bash-3.2-portable `kill -0` poll, no `wait -n`) → merge-ready ⇒ merge coordinator under merge-lease → release issue-lease → refill slots until the queue is empty. All external steps (queue / launch / result / merge) are env-injectable for deterministic testing (`FA_QUEUE_CMD` / `FA_LAUNCH_CMD` / `FA_RESULT_CMD` / `FA_MERGE_CMD`); the default launcher runs `FA_HANDOFF=1 claude -p "/atdd-kit:autopilot <i> --hand-off"`. Live-validated with real `claude -p` workers. |
