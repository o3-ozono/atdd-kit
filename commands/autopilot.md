...
PO（プロダクトオーナー）が team-lead として常駐し、discover から merge まで一気通貫で Issue を完遂する。タスクタイプに応じてエージェント構成を切り替える。
...
PO が Stakeholder（ユーザー）と対話して要件を探索し、AC を導出する。全タスクタイプ共通。
...
PO + task-type-specific agents で draft AC をレビューする。Agent Composition Table の Phase 1 列に従ってエージェントを spawn する。
...
1. PO が単独でスコープ・完了条件・テーマ分割・人数を設計する。
