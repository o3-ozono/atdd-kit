...
<!-- AT lifecycle: planned → draft → green → regression
     各エントリには状態マーカー [draft] / [draft] / [green] / [regression] を付与する。
     状態は実装の進行に合わせて更新する。
...
- [x] [regression] AT-100: AT-006 が撤去されている
...
- [x] [regression] AT-101: AT-006 撤去で入れ子フルスイート再実行という高速化阻害要因が構造的に消える
...
- [x] [regression] AT-110: 残存 AT が #271 の回帰意図を構造的に担保する
...
- [x] [regression] AT-111: フルスイート green は CI が担保する（撤去の前提）
...
- [x] [regression] AT-200: コア数検出のフォールバック連鎖がマシン非依存に成立する
...
- [x] [regression] AT-201: GNU parallel 等の外部依存なしで動作する
...
- [x] [regression] AT-210: `--all` で全件、`--impact --base <ref>` で影響範囲を選択する
...
- [x] [regression] AT-211: 重み均衡ファイルシャーディングで全件が検出コア数だけ並列分割される
...
- [x] [regression] AT-212: シャードの失敗がランナー全体の失敗に集約される
...
- [x] [regression] AT-302: reviewing-deliverables は line-budget を超えずリンク参照で担保する
...
- [x] [regression] AT-301: claude 系 e2e が影響度基準に統合されている
...
- [x] [regression] AT-310: live e2e の実行条件が影響度基準で文書化されている
...
- [x] [regression] AT-311: test-execution-policy.md が言語ポリシー（English only）を満たす
...
- [x] [regression] AT-312: test-execution-policy.md が methodology/README.md に登録されている
...
- [x] [regression] AT-320: フェーズ別ポリシーが配布メソドロジーにドクトリンとして明文化されている
...
- [x] [regression] AT-400: plugin.json version が CHANGELOG 最新リリース見出しと一致する
