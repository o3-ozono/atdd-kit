...
- [ ] [green] AT-347-1a: 末尾空白付き glob が trim されて fnmatch 一致する
...
- [ ] [green] AT-347-1b: 末尾空白付き `rules:` キーが正しくセクション開始として解釈される
...
- [ ] [green] AT-347-1c: 誤インデント（4-space / tab）がインデント規約に言及する診断エラーになる
...
- [ ] [green] AT-347-2a: `-` 始まりの `--base` は fail-closed で弾かれる
...
- [ ] [green] AT-347-2b: `--output=/path` 相当の敵対的 base で任意ファイル truncate が起きない
...
- [ ] [green] AT-347-3a: `setup-web.md` に既存 config 検出と上書き警告・保持が記載されている
  - Given: `commands/setup-web.md`
  - When: ドキュメントを読む
  - Then: 既存 `config/impact_rules.yml` 検出時に上書き警告を出し既存ファイルを保持（推奨動作）する手順が記載されている

- [ ] [green] AT-347-3b: `setup-ios.md` に同等の保護手順が記載されている
...
- [ ] [green] AT-347-5e: `DEVELOPMENT.md` に `mcp_servers` の Zero Dependencies carve-out がある
...
- [ ] [green] AT-347-6c: `AT-323-004b` が `--impact` 必須に厳格化されている
...
- [ ] [green] AT-347-6d: `AT-323-001b` / `AT-323-001c` が content assert に強化されている
...
- [ ] [green] AT-347-6e: 変更した SKILL.md の BATS 証跡が記録されている
