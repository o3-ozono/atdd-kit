...
- [x] [regression] AT-001: 同一 step の [PASS, PASS, FAIL] ログで check_stuck が continue を返す
...
- [x] [regression] AT-002: 同一 step の [PASS(fp X), FAIL(fp X)] で check_sameness が continue を返す
...
- [x] [regression] AT-003: step なし呼び出しで PASS 行が衝突源にならない
...
- [x] [regression] AT-004: 同一 step の FAIL 行反復は従来どおり halt する
...
- [x] [regression] AT-005: lib コメント・SKILL.md・iron-law doc・skills/README・lib/README が「同一 step の FAIL 行のみ」を記載する
...
- [x] [regression] AT-006: 同一 step の [FAIL(fp A), PASS, FAIL(fp A)] で check_sameness が halt を返す
