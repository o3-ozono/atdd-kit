...
- [ ] [planned] AT-341-A-1: SKILL.md が独立スキル契約（Trigger / Input / boundary / 本体非改変）を満たす
...
- [ ] [planned] AT-341-B-1: 導出可能な要件は自律ドラフトし、人間判断点だけを全 Issue 横断 1 バッチで提示する
...
- [ ] [planned] AT-341-B-2: 人間の対話回数が Issue 件数に依存しない定数回に保たれる
...
- [ ] [planned] AT-341-C-1: K スロット下で issue-lease を取りつつ busy Issue をスキップして dispatch する
...
- [ ] [planned] AT-341-C-2: worker は worktree 隔離＋プラグイン設定播種で起動される
...
- [ ] [planned] AT-341-C-3: worker lease は 3 経路すべてで解放される
...
- [ ] [planned] AT-341-D-1: keystone→後続の実装順序を共有真実源に記録しその順で worker を進める
...
- [ ] [planned] AT-341-E-1: 覆りうる点だけを選別提示し、ゼロなら最終承認をスキップする
...
- [ ] [planned] AT-341-E-2: AL-1 三ゲート不変条件と整合し Gate ③（merge）は full-autopilot 側で不変
...
- [ ] [planned] AT-341-F-1: batch-discovery の各変更が deterministic な bats AT で検証される
