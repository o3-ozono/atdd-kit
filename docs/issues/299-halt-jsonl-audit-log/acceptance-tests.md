...
- [x] [green] AT-299-1: `record_halt` が JSONL に終端 HALT レコードを追記する
...
- [x] [green] AT-299-2: 終端 HALT レコードが step・reason・findings_digest・timestamp を全て保持する
...
- [x] [green] AT-299-3: `record_iteration` の出力行に timestamp が含まれる
...
- [x] [green] AT-299-4: timestamp 付与後も fingerprint と収束レール決定論が不変
  - Given: 同一の失敗ペイロード
...
- [x] [green] AT-299-5: 範囲外 reason の終端レコードは拒否される
...
- [x] [green] AT-299-5b: autopilot SKILL.md が終端レコードを収束失敗系 halt のみに配線する
...
- [x] [green] AT-299-6: 終端レコード追記が「ログ単独 stage + commit」として配線される
...
- [x] [green] AT-299-7: 終端レコード追記後も return 値は廃止されない
...
- [x] [green] AT-299-8: HALT 行は recorded カウンタを増やさず・追記後に integrity 再走が無い
  - Given: `skills/autopilot/SKILL.md` の収束失敗系 halt 経路
