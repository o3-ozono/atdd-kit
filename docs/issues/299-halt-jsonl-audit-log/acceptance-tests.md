...
- [x] [regression] AT-299-1: `record_halt` が JSONL に終端 HALT レコードを追記する
...
- [x] [regression] AT-299-2: 終端 HALT レコードが step・reason・findings_digest・timestamp を全て保持する
...
- [x] [regression] AT-299-3: `record_iteration` の出力行に timestamp が含まれる
...
- [x] [regression] AT-299-4: timestamp 付与後も fingerprint と収束レール決定論が不変
  - Given: 同一の失敗ペイロード
...
- [x] [regression] AT-299-5: 範囲外 reason の終端レコードは拒否される
...
- [x] [regression] AT-299-5b: autopilot SKILL.md が終端レコードを収束失敗系 halt のみに配線する
...
- [x] [regression] AT-299-6: 終端レコード追記が「ログ単独 stage + commit」として配線される
...
- [x] [regression] AT-299-7: 終端レコード追記後も return 値は廃止されない
...
- [x] [regression] AT-299-8: HALT 行は recorded カウンタを増やさず・追記後に integrity 再走が無い
  - Given: `skills/autopilot/SKILL.md` の収束失敗系 halt 経路
...
## AT-299-9: audit-halt agent の haltResult が null-safe パターンで取り出される（FS-7）

- [x] [regression] AT-299-9: audit-halt agent 呼び出し結果が null-safe パターン（optional chain）で参照される
  - Given: `skills/autopilot/SKILL.md` の `audit-halt:step` ラベル付き agent() 呼び出し箇所
  - When: 構造 pin（grep）を実行する
  - Then: agent() 戻り値を変数 `haltResult` で受け取り（`const haltResult = await agent(...)`）、`haltResult?.haltRecorded` で optional chain アクセスしている。直接分割代入 `const { haltRecorded } = await agent(...)` は `null` 返却時に TypeError になるため存在しない（#292 FS-5 パターンと同型の null ガード）
  - verify: `bats tests/test_autopilot_skill.bats`
