...
- [x] [green] AT-001: Execution Mode 節の Review step bullet がレガシー記述を含まず現行実装を記述する
...
- [x] [green] AT-002: mermaid 図と導入文が Scout / Generate / Review / Verify / Aggregate を描く
...
- [x] [green] AT-003: 旧機構（固定 roster / serial / 47 基準）前提の記述が workflow-detail.md のどこにも残らない
...
- [x] [green] AT-004: docs-only 変更でも CHANGELOG 更新と patch version bump を伴う
...
- [x] [green] AT-005: 実装と Non-Goals 対象ファイルに変更が波及していない
  - Given: 作業ブランチと main の差分（`git diff main --name-only`）
  - When: 変更ファイル一覧を検査する
  - Then: 変更が `docs/workflow/workflow-detail.md` / `CHANGELOG.md` / `.claude-plugin/plugin.json` / `tests/`（回帰 pin + README 同期）/ `docs/issues/269-*`（Issue 成果物）に限定され、`skills/reviewing-deliverables/SKILL.md` と `agents/` 配下に変更がない（許可リストは `tests/acceptance/AT-269.bats` にエンコード済み）。agents/ レガシー記述は別 Issue が起票され追跡されている（手動検証: #271 を Gate ① で起票済み — 実行可能テストにはエンコードしない）
