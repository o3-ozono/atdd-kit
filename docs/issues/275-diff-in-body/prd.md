...
- **AC-3（ハンドオフ）**: マージゲート引き継ぎ（step 5）では、実装 diff（per-file stat = `git diff --stat` 形式のサマリ + key hunks）を本文へインライン提示する。green ステータスの要約だけにしない。
...
- fail-closed: 再提示の入力 `rejectionFindings` は空配列を拒否する（`[]` は JS で truthy かつ `.some()` が空虚に false のため、ガードなしでは finding ゼロの再提示パスへ素通りする — #276 round-4 レビュー検出。AC-1 の再提示判別条件の前提を成立させる制約）。
