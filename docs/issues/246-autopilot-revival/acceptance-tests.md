# Acceptance Tests: autopilot 復活 — 自律収束ループ（autopilot）
...
> 状態（2026-06-09 更新, #246 review 反映後）: **Unit Test 全 green**（test_autopilot_skill 20 / test_autopilot_convergence 20 / test_reviewing_deliverables_skill の #246 アサーション 7）。**Skill E2E** は `run-skill-e2e.sh --dry-run` で構造解決済み（autopilot.bats / reviewing-deliverables.bats）、実 claude 実行は CI（`skill-e2e-subscription`, self-hosted サブスク内）。
>
...
- [x] [green] AT-001: SKILL.md が既存 6-step skill を順に呼び、人間ゲートを最初(AC承認)と最後(merge)に絞る
  - Given: `skills/autopilot/SKILL.md` が存在する
...
- [x] [green] AT-002: 満足オラクル `AND(決定論AT緑, AC→ATカバレッジ緑, verdict=correct, 確認済みP0/P1=0)` まで generate→review→fix を反復
  - Given: `skills/autopilot/SKILL.md`
  - When: 読む（Unit: grep AND ゲート / E2E: LLM に「収束条件は何か」を問う）
  - Then: `AND(決定論 AT 緑[exit code], AC→AT カバレッジ緑, verdict=correct, 確認済み P0/P1 findings=0)` と generate→review→fix ループ、満たしたら次 step へ進む記述がある

## AT-003: 安全に失敗する (F3)

- [x] [green] AT-003: 非収束検出と escalation、verdict の JSONL 永続化
  - Given: `skills/autopilot/SKILL.md` + `lib/autopilot_convergence.sh`
...
- [x] [green] AT-006: 安全レールが pure bash、SKILL.md が行数 budget 内
  - Given: `lib/autopilot_convergence.sh` + `skills/autopilot/SKILL.md`
...
- [x] [green] AT-007: Unit + Skill E2E + lib + verdict の BATS が存在
  - Given: `tests/`
  - When: ファイルを確認（Unit）
  - Then: `tests/test_autopilot_skill.bats` / `tests/e2e/autopilot.bats` / `tests/test_autopilot_convergence.bats` が存在し、`tests/test_reviewing_deliverables_skill.bats` に verdict アサーションがある
