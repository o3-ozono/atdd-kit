...
- [ ] [planned] AT-355-F8-1: record_red_evidence が test SHA と impl baseline SHA の両方を red.jsonl へ記録する
...
- [ ] [planned] AT-355-F8-2: check_red_evidence は git log 考古学ではなく red.jsonl の記録値で判定する
...
- [ ] [planned] AT-355-F8-3: 記録の無い／改竄された test SHA で fail-closed
...
- [ ] [planned] AT-355-F8-4: running-atdd-cycle の record_red_evidence 呼び出し契約が impl SHA 記録を含む
...
- [ ] [planned] AT-355-F8-5: autopilot SKILL の red-gate 手順から SHA 推測（git log 考古学）が排除されている
...
- [ ] [planned] AT-355-F2-1: record_halt が gate-unverifiable reason を受理する
  - Given: 一時 JSONL ログ
  - When: `record_halt` を reason `gate-unverifiable` で呼ぶ
  - Then: `outcome":"HALT"` かつ `reason":"gate-unverifiable"` の HALT 行が追記される

- [ ] [planned] AT-355-F2-2: 未知 reason は従来どおり拒否される（enum 不変条件）
  - Given: 一時 JSONL ログ
...
- [ ] [planned] AT-355-F1-1: autopilot が成果物未完成と機構自己検証失敗を別クラスとして扱う
...
- [ ] [planned] AT-355-F1-2: demonstrably-done だが機構未確証なら gate-unverifiable へ分岐する
...
- [ ] [planned] AT-355-F3-1: N=3 パネルと 2/3 majority 採用が Aggregate 規則に明記される
...
- [ ] [planned] AT-355-F3-2: 単一レンズの単独判定は severity を一段下げる
...
- [ ] [planned] AT-355-F4-1: CONVERGED の二条件が記述される
...
- [ ] [planned] AT-355-F4-2: 最大ラウンド数上限が設けられている
  - Given: 同上 SKILL
  - When: ラウンド制御を読む
...
- [ ] [planned] AT-355-F5-1: Scout が PRD/US から境界を抽出し out-of-scope を分離する
...
- [ ] [planned] AT-355-F6-1: 意図的トレードオフは合議で不当判定されない限り再提出されない
  - Given: `skills/reviewing-deliverables/SKILL.md`
  - When: 設計判断の扱いを読む
...
- [ ] [planned] AT-355-F7-1: レンズ横断マージ後に 1 回だけ severity を付与する
...
- [ ] [planned] AT-355-C1-1: demonstrably-done が単一脆弱 signal に veto されて MAX 空転しない
...
- [ ] [planned] AT-355-C1-2: 機構自己検証失敗が早期 escalation される
  - Given: 機構の自己検証が失敗するシナリオ
  - When: 収束ループを回す
  - Then: MAX_ITERATIONS まで回さず `gate-unverifiable` で早期に人間 escalation する

- [ ] [planned] AT-355-C1-3: review が有限ラウンド内で CONVERGED へ到達する
...
- [ ] [planned] AT-355-C2-1: redObserved 判定がイテレーション数・履歴形状に依存しない
