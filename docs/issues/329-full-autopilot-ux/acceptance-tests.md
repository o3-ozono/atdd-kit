...
- [ ] [planned] AT-329-0a: development.yml で AC/サブタスク/完了条件/User Story が optional
...
- [ ] [planned] AT-329-0b: 意図3点フィールドは required（フィールドマッピング確定）
  - Given: `templates/issue/{ja,en}/development.yml` が存在する
...
- [ ] [planned] AT-329-0c: always-sync 経路が壊れていない
...
- [ ] [planned] AT-329-1a: 走行中に追加された ready-to-go を現セッションが拾う
...
- [ ] [planned] AT-329-1b: 同一 issue を二重起動しない
...
- [ ] [planned] AT-329-2a: 通知先未設定なら起動時に警告
  - Given: `FA_NOTIFY_CMD` が未設定
  - When: `run()` を起動する
  - Then: ログに「通知先未設定」警告が1回出力され、本体は停止しない

- [ ] [planned] AT-329-2b: 通知先設定済みなら確認ログ
...
- [ ] [planned] AT-329-3a: (consume) ラベル不在なら failed に倒れる
...
- [ ] [planned] AT-329-3b: (consume) 自己申告＋ラベル両立で merge-ready
  - Given: worker stdout が `is_error:false` かつ `gh issue view` stub が `merge-ready` ラベル存在を返す
  - When: `__default_result <issue>` を評価する
  - Then: 出力は `merge-ready`

- [ ] [planned] AT-329-3c: (produce) システムが merge-ready ラベルを実際に生成する
...
- [ ] [planned] AT-329-4a: 必須チェックと override が記述されている
...
- [ ] [planned] AT-329-5a: ready-to-go 前提が正典 DoR に一致
...
- [ ] [planned] AT-329-CS1a: 全 bats スイートが green
  - Given: US-0〜US-5 の変更と各 AT bats が実装済み
  - When: `bats tests/`（および該当時 `bats addons/*/tests/`）を実行する
  - Then: 全テストが green（回帰なし）

- [ ] [planned] AT-329-CS1b: version と CHANGELOG の invariant（[regression] 候補）
