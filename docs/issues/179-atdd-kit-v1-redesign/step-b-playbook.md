...
| 4 | 本実装 + テスト | `skills/<name>/SKILL.md`, `tests/test_<name>_skill.bats`, `tests/e2e/<name>.bats`, `CHANGELOG.md`, AT を `[planned]→[green]`, `tests/test_v1_skill_skeletons.bats` から該当行削除 | 全部まとめて 1 コミット（ここは AI の機械的作業に近いので一括 OK） | `e441e787` |
