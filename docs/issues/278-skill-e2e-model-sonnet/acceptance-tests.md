...
- [ ] [green] AT-001: `tests/e2e/*.bats` 全 11 ファイルの `_run_claude` が `--model "${E2E_MODEL}"` を渡す
...
- [ ] [green] AT-002: `E2E_MODEL="${SKILL_E2E_MODEL:-sonnet}"` 定義により上書き可能・デフォルト sonnet
...
- [ ] [green] AT-003: 構造検証テストが「全 e2e ファイルの `--model` 指定」を機械検証する
...
- [ ] [green] AT-004: `tests/README.md` の Skill E2E Tests 節がデフォルトモデルと上書き方法を伝える
...
- [ ] [green] AT-005: `timeout` / `gtimeout` / フォールバックの 3 分岐すべてに同一フラグが付与される
...
- [ ] [green] AT-006: モデル設定が `TIMEOUT_SECS` と同じ命名規約・同じ上書き方式・同じ配置で定義される
...
- [ ] [green] AT-007: テストハーネス変更でも CHANGELOG 更新と patch version bump を伴う
...
- [ ] [green] AT-008: ハーネス共通化・fixtures・実行スクリプトに変更が波及していない
