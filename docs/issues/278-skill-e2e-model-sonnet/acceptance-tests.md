...
- [ ] [planned] AT-001: `tests/e2e/*.bats` 全 11 ファイルの `_run_claude` が `--model "${E2E_MODEL}"` を渡す
...
- [ ] [planned] AT-002: `E2E_MODEL="${SKILL_E2E_MODEL:-sonnet}"` 定義により上書き可能・デフォルト sonnet
...
- [ ] [planned] AT-003: 構造検証テストが「全 e2e ファイルの `--model` 指定」を機械検証する
...
- [ ] [planned] AT-004: `tests/README.md` の Skill E2E Tests 節がデフォルトモデルと上書き方法を伝える
...
- [ ] [planned] AT-005: `timeout` / `gtimeout` / フォールバックの 3 分岐すべてに同一フラグが付与される
...
- [ ] [planned] AT-006: モデル設定が `TIMEOUT_SECS` と同じ命名規約・同じ上書き方式・同じ配置で定義される
...
- [ ] [planned] AT-008: ハーネス共通化・fixtures・実行スクリプトに変更が波及していない
