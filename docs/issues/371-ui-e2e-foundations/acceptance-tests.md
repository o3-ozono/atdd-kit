...
- [ ] [planned] AT-371-1: `ui-e2e-foundations.md` が存在し必須 3 節を持つ
...
- [ ] [planned] AT-371-2: 原則 1〜4 の見出しが欠落なく存在する
...
- [ ] [planned] AT-371-3: 9 出典すべての識別語が脚注ブロックに存在する
  - Given: `## 脚注` ブロック
...
- [ ] [planned] AT-371-4: 補助原則に `[独自]` マーカーが存在する
...
- [ ] [planned] AT-371-5: 原則 2 に両流派の根拠と trade-off が併記される
  - Given: `### 原則 2`（要素の掴み方）節
...
- [ ] [planned] AT-371-6: 混在禁止が hard rule として記述される
  - Given: `### 原則 2` 節
...
- [ ] [planned] AT-371-7: `## LLM 向けルール集` 節に `[hard rule]` 付き命令形ルールが 4 原則分以上存在する
  - Given: `## LLM 向けルール集` 節
...
- [ ] [planned] AT-371-8: web/ios/discord の 3 addon が `ui-e2e-foundations.md` を参照する
...
- [ ] [planned] AT-371-9: 原則本文が addon ドキュメントに重複記載されていない
...
- [ ] [planned] AT-371-10: 新 BATS がフルスイートで green かつ `@covers` gate を満たす
...
- [ ] [planned] AT-371-11: ルール集の各 hard rule が 4 原則のいずれかに紐づく
