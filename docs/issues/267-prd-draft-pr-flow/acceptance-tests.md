...
- [x] [regression] AT-001: Execution Mode 節からレガシー規定が消え、Draft PR ベースの規定が存在する
...
- [x] [regression] AT-002: Flow の順序が承認前書き込み + Draft PR 提示に変更されている
...
- [x] [regression] AT-003: Dialog economy 節に PR 差分ベース提示の規定が存在する
...
- [x] [regression] AT-004: 全文展開禁止の規定が両スキルに存在する
...
- [x] [green] AT-005: 変更文言に対応する pin が追加され、全スイートが green
  - エビデンス（一回性のコマンド検証）: `bats tests/` → 1125 tests / not ok 0 件 / exit code 0（2026-06-11, Step 4 実行）
...
- [x] [green] AT-006: rules/atdd-kit.md は無変更で、同期規定は保持される
  - エビデンス（一回性のコマンド検証）: `git diff main -- rules/atdd-kit.md` → 出力 0 行（2026-06-11, Step 4 実行）。新規定はいずれも成果物本体の置き場所のみを定め、autopilot SKILL.md の追記 bullet は全チャネル同期の維持を明文化
