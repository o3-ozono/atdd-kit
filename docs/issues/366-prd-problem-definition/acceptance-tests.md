...
- [ ] [draft] AT-366-1: `templates/docs/issues/prd.md` に新 4 要素の見出しが順に存在する
...
- [ ] [draft] AT-366-2: `## 2. 問題定義と背景` 節に「事実」欄と「課題」欄が別々に存在する
  - Given: 再編後の PRD テンプレ
...
- [ ] [draft] AT-366-3: テンプレ内コメントに 4 原則が出現し出典/独自マーカーが付く
  - Given: 再編後の PRD テンプレ
...
- [ ] [draft] AT-366-4: 4 種の anti-pattern が警告として記述されている
  - Given: 再編後の PRD テンプレ
...
- [ ] [draft] AT-366-5: `## 3. ゴールと成功指標` に内部完了条件を禁じる規律がある
  - Given: 再編後の PRD テンプレ
...
- [ ] [draft] AT-366-6: `## 4. 機能要件` に Non-Goals 統合欄と優先度列プレースホルダーがある
  - Given: 再編後の PRD テンプレ
  - When: 機能要件節を走査する
...
- [ ] [draft] AT-366-8: `## Open Questions` に状態管理ガイダンスがある
  - Given: 再編後の PRD テンプレ
...
- [ ] [draft] AT-366-9: SKILL.md の Flow が 4 原則対応の問いかけと 1 質問ずつ規律を含む
...
- [ ] [draft] AT-366-10: 既存 `docs/issues/*/prd.md` が本 Issue で書き換えられていない
  - Given: 本 Issue の変更差分
...
- [ ] [draft] AT-366-11: plugin.json の version が CHANGELOG 最上位リリース見出しと一致する
