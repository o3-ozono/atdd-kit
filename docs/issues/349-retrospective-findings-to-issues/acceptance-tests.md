...
- [ ] [planned] AT-349-1: SKILL.md Flow Step 5 に「壊れた/異常なメトリクス → `type:bug` 起票」の分類が明記されている
...
- [ ] [planned] AT-349-2: SKILL.md に friction point / improvement candidate → skill-fix 起票と `atdd-kit:skill-fix` ルート案内がある
...
- [ ] [planned] AT-349-3: SKILL.md に「非アクション → スキップ（起票不要）」と判定基準が明記されている
  - Given: SKILL.md の Flow Step 5 セクション
...
- [ ] [planned] AT-349-4: SKILL.md に起票 Issue 番号を retrospective サマリへ追記し terminal と Issue/PR コメント両方へ出力する旨がある
  - Given: SKILL.md の Flow Step 5 セクション
  - When: 起票後のサマリ反映手順を読む
...
- [ ] [planned] AT-349-5: SKILL.md に auto-routing を行わず actionable 判定を人間が最終確認する旨がある
  - Given: SKILL.md の Flow Step 5 セクション
...
- [ ] [planned] AT-349-6: `templates/docs/issues/retrospective.md` の "Improvement Candidates" 節が 3 分類要点・番号追記・SKILL.md 参照へ更新され、旧消極注記が残っていない
...
- [ ] [planned] AT-349-7: 本 Issue の変更が `scripts/retrospective.sh` に一切の差分を持ち込んでいない
