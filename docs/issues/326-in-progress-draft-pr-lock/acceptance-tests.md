...
- [ ] [green] AT-326-1: `gh pr create --draft` を hook が検知し、リンク Issue へ in-progress を付与する
...
- [ ] [green] AT-326-2: PR body の `Closes #<N>` と branch 名プレフィックス `<N>-...` の両方から Issue 番号を解決する
...
- [ ] [green] AT-326-3: `--draft` 無しの PR 作成や無関係 Bash では付与が起きない
  - Given: hook が有効
...
- [ ] [green] AT-326-4: `gh pr close` を hook が検知し、リンク Issue から in-progress を除去する
...
- [ ] [green] AT-326-5: 付与・除去が冪等で害なく no-op になる
  - Given: hook が有効
...
- [ ] [green] AT-326-6: 異常入力でも hook は必ず allow（exit 0）し副作用ゼロ
  - Given: hook が有効
...
- [ ] [green] AT-326-7: open PR / in-progress を持つ Issue は dispatch select 対象から除外される
...
- [ ] [green] AT-326-8: busy Issue は lease を取得せずスキップされる（揮発 lease 消失後でも再 dispatch しない）
...
- [ ] [green] AT-326-9: プリフィルタ追加後も `cmd_select` は lease-store 合成の純粋ロジックのまま（GitHub 問い合わせを内蔵しない）
...
- [ ] [green] AT-326-10: 新 hook が hooks.json に登録され README・配布整合が保たれる
