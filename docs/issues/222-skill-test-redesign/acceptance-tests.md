...
## AT-001: 旧用語が CHANGELOG.md 以外から消えている (FS-1)

- [ ] [planned] AT-001: terminology grep test
  - **Given:** atdd-kit リポジトリ HEAD
  - **When:** `grep -rn "SAT\b\|Skill Acceptance\|Fast layer\|Integration layer\|L1 BATS\|L2 Fast\|L3 Integration" --include="*.md" --include="*.sh" --include="*.bats" --include="*.yml" .` を CHANGELOG.md 除外で実行
  - **Then:** 0 hit (exit 1)

## AT-002: path-based 影響範囲算定 — 単一 skill 変更 (FS-2)

- [ ] [planned] AT-002: impact mapping for skill change
...
- [ ] [planned] AT-003: impact mapping for shared assets
...
- [ ] [planned] AT-004: runner log fields
...
- [ ] [planned] AT-005: reviewer handoff log spec documented
...
- [ ] [planned] AT-006: e2e structure example documented
  - **Given:** `docs/testing-skills.md` 書き換え後
...
- [ ] [planned] AT-007: cross-issue propagation
...
- [ ] [planned] AT-008: no evals dependency
...
- [ ] [planned] AT-009: git sha mandatory in log
...
- [ ] [planned] AT-010: full-run gating
