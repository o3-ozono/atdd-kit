...
- [ ] [planned] AT-001: `rejectionFindings` args 配管が存在し、iteration 1 の generate プロンプトへ verbatim で到達する
...
- [ ] [planned] AT-002: 不正な `rejectionFindings` は 1 イテレーションも走らせず throw する
...
- [ ] [planned] AT-004: 非 'ok' 応答が成果物セット全体の差し戻しとして扱われる規律が SKILL.md Flow 節にある
...
- [ ] [planned] AT-005: 複合コメントのセクション単位分割規律が SKILL.md にある
  - Given: SKILL.md の Flow 節 step 3（design-approval gate）
  - When: BATS pin が SKILL.md を検査する
