...
- [ ] [planned] AT-001: 別 step・別 phase のログ行が sameness の比較対象に混入しない
...
- [ ] [planned] AT-002: step スコープ化しても検出力が後退しない（sameness）
...
- [ ] [planned] AT-003: step 省略時はログ全体を単一系列とする現行挙動が維持される
...
- [ ] [planned] AT-004a: 別 step 行は window の母集団から除外される
...
- [ ] [planned] AT-004b: 同一 step 内の真の停滞（flatline / oscillation）は halt
...
- [ ] [planned] AT-005: canonical Workflow script が rails 呼び出しに現在の step を渡している
...
- [ ] [planned] AT-006: audit payload が oracle 状態込み JSON に拡張されている
...
- [ ] [planned] AT-007: 両 BATS スイートがフル実行で green
