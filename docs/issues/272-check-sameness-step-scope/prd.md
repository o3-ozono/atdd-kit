...
1. **修正範囲: step スコープ化のみ(案 a)か、gate 状態 fingerprint も併せる(案 a+b)か** — 案 a 単独ではクロス phase 誤発火は解消するが、同一 step 内で「review クリーンのまま uncovered が 3→1 件に減る」正当な前進を依然 sameness と誤認する。本 PRD は **a+b 両方**を提案(推奨)。
