...
- [ ] [planned] AT-001: session-start の Recommended Tasks 出力テンプレートに「推奨経路」列が存在する
...
- [ ] [planned] AT-002: Task Recommendation Rules の Step 3 でハイブリッド判定（決定的ガードレール + LLM 判断）と両信号が定義される
...
- [ ] [planned] AT-003: 判定が曖昧な Issue は autopilot（フルフロー）にフォールバックすると規定される
...
- [ ] [planned] AT-004: 推奨経路はあくまで推奨であり auto-route されず、ユーザーが最終選択する
  - Given: `### Task Recommendation Rules` の Step 3
  - When: 経路推奨の拘束力を確認する
...
- [ ] [planned] AT-005: express の明示起動 + APPROVAL-GATE + scope-overflow + CI ゲートが変更されずに存続する
...
- [ ] [planned] AT-006: 推奨経路の構造アサーションが追加され既存スイートが green を維持し、autopilot SKILL.md は変更されない
...
- [ ] [regression] AT-007: plugin.json の version が CHANGELOG 最上位リリース見出しと一致する
