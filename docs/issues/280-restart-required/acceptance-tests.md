...
- [x] [green] AT-001: 新版インストール済み + 旧版ロード中で RESTART_REQUIRED が出力される
...
- [x] [green] AT-002: ロード版がマーカー版より古いとき STALE_SESSION が出力されマーカーは更新されない
...
- [x] [green] AT-003: session-start が STALE_SESSION を受けたとき E2 Auto-Sync を実行せず再起動を促す
...
- [x] [green] AT-004: UPDATED 経路で CACHED 見出しが CHANGELOG に無いとき件数が UNKNOWN になる
...
- [x] [green] AT-005: session-start が RESTART_REQUIRED を再起動メッセージとしてレポートに反映する
...
- [x] [green] AT-006: installed_plugins.json が無い/パース不能/該当エントリなしでも従来トークンが壊れない
...
- [x] [green] AT-007: ロード版 < マーカー版 かつ installed 版 > ロード版のとき STALE_SESSION が優先される
...
- [x] [green] AT-008: RESTART_REQUIRED / STALE_SESSION 後に再起動した次セッションで正しい UPDATED が出る
...
- [x] [green] AT-009: すべての検知がネットワークアクセスなしでローカルファイルのみで完結する
...
- [x] [green] AT-010: 既存 BATS スイート green + 新規分岐テスト追加 + CHANGELOG/バージョン bump
