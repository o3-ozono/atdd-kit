...
- [ ] [planned] AT-351-1: `commands/bugfix.md` が存在し fixing-bugs ルートへ配線され、`commands/autofix.md` は存在しない
...
- [ ] [planned] AT-351-2: live documentation の `autofix` 参照が `bugfix` へ更新されている
...
- [ ] [planned] AT-351-3: `tests/acceptance/AT-308.bats` のパス pin と `/atdd-kit:autofix` grep が新コマンド名へ更新され green
  - Given: rename 後のリポジトリ
...
- [ ] [planned] AT-351-4: 命名のみの変更でルートのフロー・ゲート・オラクルが不変、既存テストスイートが green
  - Given: rename 後のリポジトリ
...
- [ ] [planned] AT-351-5: plugin.json version が CHANGELOG 最新リリース見出しと一致し、#351 エントリが追記されている
  - Given: rename 後のリポジトリ
