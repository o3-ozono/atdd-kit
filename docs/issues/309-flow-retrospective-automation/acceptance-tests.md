...
- [ ] [planned] AT-309-1: 非 express の Issue 完了時に振り返りが既定 ON で軽量に起動する
...
- [ ] [planned] AT-309-2: express 経路の完了では振り返りが起動しない（構造的スキップ）
...
- [ ] [planned] AT-309-3: ターン数がフェーズ別内訳付きで集計される
...
- [ ] [planned] AT-309-4: harness 提供値を一次ソースにトークン量が出力され、欠落時は best-effort 注記が付く
...
- [ ] [planned] AT-309-5: 直近マージ PR を対象に diff 行数で正規化した前回比が出力される
...
- [ ] [planned] AT-309-6: 永続化された gate rejection シグナルを起点に摩擦点がゲート別に分類される
...
- [ ] [planned] AT-309-7: skill-fix 候補がサマリ列挙され、自動起票は行われない
...
- [ ] [planned] AT-309-8: retrospective.md 生成・横断 JSONL 追記・全チャネル同期が成立する
...
- [ ] [planned] [regression] AT-309-9: 機能 PR でバージョンと CHANGELOG が整合する
