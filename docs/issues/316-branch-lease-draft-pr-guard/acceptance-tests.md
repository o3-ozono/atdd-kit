...
- [ ] [planned] AT-001: session-start SKILL.md が Draft PR を read-only 表示に限定する
...
- [ ] [planned] AT-002: CONFLICTING rebase 推奨が非 Draft ＋ @me 限定として記述される
...
- [ ] [planned] AT-003: 別セッション保有 Draft ブランチへの write-back が deny される
...
- [ ] [planned] AT-004: checkout / switch / ローカル rebase / main 操作はブロックされない
...
- [ ] [planned] AT-005: 非 main ブランチ push でリース未取得なら自セッション名義で取得する
...
- [ ] [planned] AT-006: ATDD_BRANCH_LEASE_FORCE=1 で hard block を上書きできる
...
- [ ] [planned] AT-007: lease が共有 store に branch キーで保存されクロスセッション参照できる
...
- [ ] [planned] AT-008: TTL 超過リースはアクセス時に掃除されブロックを生まない
...
- [ ] [planned] AT-009: フック単体 ＋ E2E ＋ session-start BATS ＋ README が回帰保護を構成する
...
- [ ] [planned] AT-010: plugin.json の version が CHANGELOG 最上位 release 見出しと一致する
