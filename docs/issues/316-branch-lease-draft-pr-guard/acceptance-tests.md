...
- [x] [regression] AT-001: session-start SKILL.md が Draft PR を read-only 表示に限定する
...
- [x] [regression] AT-002: CONFLICTING rebase 推奨が非 Draft ＋ @me 限定として記述される
...
- [x] [regression] AT-003: 別セッション保有 Draft ブランチへの write-back が deny される
  ...
  - Then: フックが `permissionDecision: "deny"` JSON を stdout に返して exit 0 で終了する（Claude Code の PreToolUse deny 機構は exit 0 ＋ deny JSON であり、exit 非ゼロは hook 実行エラー扱いでツールを確実にはブロックしない）。deny 理由に別セッションが作業中である旨を含む

## AT-004: 非 write-back 操作と main を通過させる（FS-4 / Layer 2）

- [x] [regression] AT-004: checkout / switch / ローカル rebase / main 操作はブロックされない
...
- [x] [regression] AT-005: 非 main ブランチ push でリース未取得なら自セッション名義で取得する
...
- [x] [regression] AT-006: ATDD_BRANCH_LEASE_FORCE=1 で hard block を上書きできる
...
- [x] [regression] AT-007: lease が共有 store に branch キーで保存されクロスセッション参照できる
...
- [x] [regression] AT-008: TTL 超過リースはアクセス時に掃除されブロックを生まない
...
- [x] [regression] AT-009: フック単体 ＋ E2E ＋ session-start BATS ＋ README が回帰保護を構成する
...
- [x] [regression] AT-010: plugin.json の version が CHANGELOG 最上位 release 見出しと一致する
