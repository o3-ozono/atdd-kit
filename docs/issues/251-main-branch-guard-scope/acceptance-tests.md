...
- [x] [green] AT-001: セッション cwd と無関係な別 git リポジトリ配下のファイル編集が allow される
...
- [x] [green] AT-002: cwd が main にあっても、feature worktree 配下への Write が allow される
...
- [x] [green] AT-003a: main worktree 内ファイルへの直接編集は deny される
...
- [x] [green] AT-003b: 既存 allow-list 例外は不変（リポジトリ内パスで段階 (d) を検証）
  - Given: cwd はプロジェクトリポジトリ（main ブランチ）。新判定フローではリポジトリ外パス（`/tmp`・`/dev/null` 等）は段階 (b) で allow されるため allow-list 照合 (d) を踏まない。照合 (d) の検証には **HOME override**（fake `HOME=$WORK`、既存 AC2 #181 と同方式）でリポジトリ内に解決される `~/.claude` / `~/.config` パスを使う
  - When: リポジトリ内に解決される `~/.claude`・`~/.config` 配下のパスを `file_path` とするフック JSON を渡す（`/tmp`・`/dev/null` 等のリポジトリ外 allow-list パスは AC1(#181) 系テストで観測結果 `{}` の不変を確認）
  - Then: いずれも stdout が `{}`（main 上のリポジトリ内パスでも allow-list 照合 (d) が従来どおり機能する）

## AT-004: ブランチ判定が py 側の対象ファイル基準に集約されている（US-4）

- [x] [green] AT-004: cwd が feature ブランチでも、main worktree 内のファイル編集は deny される
...
- [x] [green] AT-005: README / CHANGELOG / plugin.json が本変更と同期している
...
- [x] [green] AT-006: git/python3 不在・非 git・malformed・detached HEAD のすべてで `{}` が返る
...
- [x] [green] AT-007: BATS スイート全件 green
