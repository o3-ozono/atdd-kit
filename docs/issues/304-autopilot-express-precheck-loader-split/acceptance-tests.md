...
- [ ] [planned] AT-101: route-eligibility.md が単一ソースとして存在する
...
- [ ] [planned] AT-102: session-start が信号を二重定義せず route-eligibility.md を参照する
...
- [ ] [planned] AT-201: 分割後の autopilot SKILL.md が行バジェット pin を満たす
...
- [ ] [planned] AT-202: 分割後も autopilot BATS 構造 pin が全 green
...
- [ ] [planned] AT-203: SKILL.md の docs ポインタが実在 doc を指す
...
- [ ] [planned] AT-204: session-start 経路判定 BATS スイートが FS-1 後も green（#302 pin の参照先追従）
...
- [ ] [planned] AT-301: express 適格 Issue で続行確認を一度だけ提示する
...
- [ ] [planned] AT-302: express 非適格 Issue では無言で従来どおり続行する
  - Given: 対象 Issue が express 適格信号を満たさない
  - When: autopilot がプリチェックを評価する
  - Then: 続行確認を提示せず、従来どおり autopilot を続行する

- [ ] [planned] AT-303: プリチェックは route-eligibility.md を判定基準として参照する
...
- [ ] [planned] AT-401: User gate は exactly three のまま（AL-1 堅持）
