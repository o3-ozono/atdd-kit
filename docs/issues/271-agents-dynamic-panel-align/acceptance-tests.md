...
- [ ] [planned] AT-001: 未使用の固定 reviewer agent が存在しない
...
- [ ] [planned] AT-002a: レガシー Usage / 固定 roster の記述が消えている
...
- [ ] [planned] AT-002b: 現行実装の 3 点構成が記述されている
  - Given: 再構成後の `agents/README.md`
  - When: 内容を検査する
...
- [ ] [planned] AT-002c: #259 モデルポリシー pin が無傷である
  - Given: 再構成後の `agents/README.md`
...
- [ ] [planned] AT-003a: docs / skills / commands / rules / ルート文書で参照 0 件
  - Given: 本 Issue の変更を適用した作業ツリー
  - When: `grep -rE 'prd-reviewer|us-reviewer|plan-reviewer|code-reviewer|at-reviewer|final-reviewer'` を `agents/ docs/ skills/ commands/ rules/ README.md README.ja.md DEVELOPMENT.md DEVELOPMENT.ja.md` に対して実行する（`docs/issues/` は歴史的記録として除外）
  - Then: ヒット 0 件である

- [ ] [planned] AT-003b: 個別文書が動的パネル記述に置換されている
  ...
  - Then: 固定 reviewer・「specialist reviewer subagents」「47 criteria」前提の記述がなく、動的レンズパネル × 並列 Workflow（#234）として説明されている

## AT-004: テスト差し替え（US-4）

- [ ] [planned] AT-004a: 旧構造テストが削除されている
  - Given: 本 Issue の変更を適用した作業ツリー
  - When: `tests/test_reviewer_subagents.bats` の存在を検査する
  - Then: ファイルが存在しない

- [ ] [planned] AT-004b: 回帰 pin テストが追加され green である
...
- [ ] [planned] AT-004c: #105 テストが削除済みファイルを参照していない
...
- [ ] [planned] AT-004d: tests/README.md が同期されている
...
- [ ] [planned] AT-005: CHANGELOG `### Removed` + minor bump
...
- [ ] [planned] AT-006: suite 全体が green で出荷される
...
- [ ] [planned] AT-007: スコープ外変更が混入していない
  - Given: `git diff main` の変更ファイル一覧
  - When: Non-Goals 対象を検査する
  - Then: `skills/reviewing-deliverables/SKILL.md` に差分がなく、`CHANGELOG.md` の差分は新エントリ追加のみ、`docs/issues/` の差分は `271-agents-dynamic-panel-align/` 配下のみ、`agents/README.md` の #259 ポリシー blockquote に差分がなく、`skills/autopilot/SKILL.md` / `skills/running-atdd-cycle/SKILL.md` からの `agents/README.md` 参照が維持されている
