...
- [ ] [draft] AT-356-1a: `collect_all_bats` が `tests/acceptance/*.bats` を収集対象に含める
...
- [ ] [draft] AT-356-1b: `--all` が失敗する acceptance テストを集約して非 0 を返す（false-green の直接 oracle・赤→緑 anchor）
...
- [ ] [draft] AT-356-2: 同一 fixture で `--all` と `--impact`(FALLBACK) がともに acceptance/ の失敗を検出して非 0 を返す
...
- [ ] [draft] AT-356-3: merge gate が `run-skill-e2e.sh --changed-files` を実行する記述を持つ
...
- [ ] [draft] AT-356-4: merge gate が e2e 全件実行（`--all`）を強制せず影響選択（`--changed-files`）である
...
- [ ] [draft] AT-356-5a: #324 `acceptance-tests.md` の `AT-210f` が新契約（acceptance/ 再帰収集）に改訂されている
...
- [ ] [draft] AT-356-5b: `collect_all_bats` のコードコメントが新スコープに整合している
...
- [ ] [draft] AT-356-5c: `docs/methodology/test-execution-policy.md` が新契約に整合している
...
- [ ] [draft] AT-356-6a: merge gate が独自 impact mapping を再実装せず既存 `run-skill-e2e.sh` を再利用する
...
- [ ] [draft] AT-356-6b: CI（pr.yml）の再帰 bats 実行が不変である
