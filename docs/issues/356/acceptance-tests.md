...
- [x] [regression] AT-356-1a: `collect_all_bats` が `tests/acceptance/*.bats` を収集対象に含める
...
- [x] [regression] AT-356-1b: `--all` が失敗する acceptance テストを集約して非 0 を返す（false-green の直接 oracle・赤→緑 anchor）
...
- [x] [regression] AT-356-2: 同一 fixture で `--all` と `--impact`(FALLBACK) がともに acceptance/ の失敗を検出して非 0 を返す
...
- [x] [regression] AT-356-3: merge gate が `run-skill-e2e.sh --changed-files` を実行する記述を持つ
...
- [x] [regression] AT-356-4: merge gate が e2e 全件実行（`--all`）を強制せず影響選択（`--changed-files`）である
...
- [x] [regression] AT-356-5a: #324 `acceptance-tests.md` の `AT-210f` が新契約（acceptance/ 再帰収集）に改訂されている
...
- [x] [regression] AT-356-5b: `collect_all_bats` のコードコメントが新スコープに整合している
...
- [x] [regression] AT-356-5c: `docs/methodology/test-execution-policy.md` が新契約に整合している
...
- [x] [regression] AT-356-6a: merge gate が独自 impact mapping を再実装せず既存 `run-skill-e2e.sh` を再利用する
...
- [x] [regression] AT-356-6b: CI（pr.yml）の再帰 bats 実行が不変である
...
`--all` を acceptance/ 再帰化すると、acceptance/ 内のメタテストが同一リポジトリに対して `--all` を呼ぶと無限再帰する（`AT-324:AT-211` で顕在化）。fail-loud な再帰ガードで構造的に防止する。

- [x] [regression] AT-356-7a: 同一 live repo への nested `--all`（`_RUN_TESTS_ALL_ACTIVE` 設定済み）が fail-loud で拒否される
  - Given: `_RUN_TESTS_ALL_ACTIVE` に対象 repo の解決済みパスが設定されている
  - When: 同一 repo に対して `run-tests.sh --all` を実行する
  - Then: 非 0 で終了し、再帰非対応のエラーメッセージを出す

- [x] [regression] AT-356-7b: 別 repo（fixture）への nested `--all` は許容される（メタテストの fixture 検証）
  - Given: `_RUN_TESTS_ALL_ACTIVE` が別 repo（active）に設定されている
  - When: green な fixture repo に対して `run-tests.sh --all` を実行する
  - Then: 再帰でないため許容され exit 0
