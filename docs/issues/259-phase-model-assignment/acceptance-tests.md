...
- [ ] [planned] AT-001: reviewing-deliverables の Workflow script で Scout / Generate / Review / Verify の `agent()` が `model: 'sonnet'` を持つ
...
- [ ] [planned] AT-002: Aggregate の `agent()` には `model` 指定がない
...
- [ ] [planned] AT-003: autopilot と running-atdd-cycle に impl phase の推奨モデルが明文化されている
...
- [ ] [planned] AT-004: 「intentionally unset」が新ポリシー + ベンチ要点に置換されている
...
- [ ] [planned] AT-005: 昇格経路がトリガー条件付きで規定されている
...
- [ ] [planned] AT-006: 変更が impl / review の subagent 指定とガイダンス文書に限定されている
...
- [ ] [planned] AT-007: 追加 pin を含む BATS スイート全体が pass する
