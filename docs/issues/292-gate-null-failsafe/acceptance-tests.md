...
- [ ] [planned] AT-001: at-gate の `agent()` 戻り値 null が atGreen=false（gate 未通過）に倒れ、クラッシュしない
...
- [ ] [planned] AT-002: coverage の `agent()` 戻り値 null が coverageOk=false・uncovered=[] に倒れ、クラッシュしない
...
- [ ] [planned] AT-003: review（verdict）の `agent()` 戻り値 null が「未収束」（overall_correctness 不一致・findings 無し）に倒れ、PASS にならずクラッシュしない
...
- [ ] [planned] AT-004: freeze（frozen）の `agent()` 戻り値 null が COMPLETED_WITH_DEBT（reason 系: freeze-error）で安全終了し、クラッシュしない
...
- [ ] [planned] AT-005: audit（rec）の `agent()` 戻り値 null が既存 recordOk 経路に合流し COMPLETED_WITH_DEBT（reason: record-error）で安全終了する
...
- [ ] [planned] AT-006: rails（r）の `agent()` 戻り値 null が halt（COMPLETED_WITH_DEBT, reason: rails-error）で安全終了し、`r.acDriftExit` 等を null 参照しない
...
- [ ] [planned] AT-007: いずれの null 経路も「収束済み / PASS」と誤判定しないことが SKILL.md に明示される
...
- [ ] [planned] AT-008: null フェイルセーフの構造アサーションが追加され、既存 BATS スイートが green を維持する
...
- [ ] [planned] AT-009: SKILL.md が行バジェット内に収まる（第 3 回引き上げ不可のため上限固定）
