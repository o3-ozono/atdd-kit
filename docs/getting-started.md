...
Setup happens automatically on the first session. The `session-start` skill auto-detects your platform (iOS, Web, Other) by scanning for project markers (e.g., `*.xcodeproj` for iOS), shows what will be installed, and asks for confirmation before proceeding.
