...
- runner のマシンで `claude`（Pro/Max ログイン済み）と `bats` が PATH に存在すること。
- atdd-kit は public のため、E2E ワークフローは **`workflow_dispatch` 限定**（write 権限者のみ起動可・fork から起動不可）。他ワークフローは `atdd-kit-e2e` ラベルを使わないこと。
