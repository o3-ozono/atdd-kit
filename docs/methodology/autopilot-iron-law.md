...
The standard Iron Laws (`rules/atdd-kit.md`) govern **human-driven** work: one Issue, one PR, a human approving every Acceptance Criterion before code is written. **autopilot** — the autonomous convergence loop owned by the `autopilot` skill — is structurally different. It *generates → objective-gate → fixes* a deliverable on its own and iterates until an **objective** satisfaction oracle (red→green AT + AC→AT coverage) is met — **never an LLM reviewer's self-judgment** (#355: self-correction without an external signal does not converge — it oscillates and plateaus; review judgment is routed to the human merge gate). A human does not approve each iteration, and one convergence cycle deliberately produces several deliverables at once.
...
On the **flaky route** (`fixing-flaky-tests`, #322) the middle gate is likewise **specialized** from design-approval to a **cause-agreement** gate: the flaky route writes no user-stories / plan / acceptance-tests spec, so its middle gate's approval target is `debugging` Step 5's **non-determinism root-cause classification (Type, flaky sub-axis, evidence 付き) ＋ the repeatedly-failing anchor ＋ failure rate (修正前 X% → 修正後 0%)** — rather than the bugfix route's single failing reproduction test. **The gate count stays three** — this is a specialization, not a removal or an added gate. The approval target is non-empty (classification + iterative failing anchor + failure rate), so AL-1's "ATDD never starts before that gate" is preserved: the determinization fix never starts before the cause-agreement gate. This middle-gate specialization for the flaky route is described identically in `docs/methodology/autopilot-design-gate.md` (the presentation contract) — both docs name the flaky middle gate the **cause-agreement** gate and the same approval target, so neither goes stale.

#### AL-3 flaky specialization (#322) — coverage term + convergence oracle

On the flaky route the standard AND oracle's `AC→AT coverage` term has **no approved AC set** to cover (no US/plan/AT spec). To keep the coverage guard — never degraded to a bare "tests pass" — the coverage term is **specialized** (not overridden; the standard route's `AC→AT coverage` is unchanged): its covered target becomes the **cause-agreement-gate-approved repeatedly-failing anchor**. The coverage gate verifies that **the repeatedly-failing anchor exists and went from red (observed at least once in N iterations) to N consecutive greens (determinization confirmed)** (the oracle anchor is real and the fix turned it deterministically green). This preserves AL-2/AL-3's external-anchor coverage guard (the loop never grades its own AT). The AND structure is objective-only (#355: no LLM-reviewer term): `AND(反復 green [deterministic, N consecutive], 反復 failing アンカー被覆 green — 反復赤→N 回連続緑 confirmed externally)`.

The flaky convergence oracle wiring is therefore: **N 回連続 green（決定化の確認）＋ 既存テスト非破壊（既存回帰なし）＋ 反復 failing アンカー 赤→N 回連続緑（= specialized coverage anchor）**, with the **middle gate = cause-agreement** (承認対象 = 非決定性の原因分類＋反復失敗率（修正前 X% → 修正後 0%）) and the **terminal gate = the User merge gate (AL-1 maintained — never auto-merge)**. **単発 green を収束としない** — N 回連続 green で決定化が確認される。
...
### AL-3 — Objective satisfaction-oracle AND gate (strengthens standard #3)
A deliverable is "done" only when the **objective** oracle `AND(red evidence observed [red-first, #334/#355], executable AT green, AC→AT coverage green [AL-2])` holds (#355 F1: no LLM-reviewer term — `overall_correctness` / blocking-findings were removed because self-judgment does not converge). Pass/fail of test/AT is decided by the **deterministic gate** (exit code / `check_red_evidence` / coverage check), never by asking an LLM whether tests "would" pass or whether the work "looks correct". Review judgment is not a convergence term; it enters at the human merge gate. No completion claim without this objective evidence.

#### AL-3 bugfix specialization (#308) — coverage term + convergence oracle
On the bugfix route the standard AND oracle's `AC→AT coverage` term has **no approved AC set** to cover (no US/plan/AT spec). To keep the coverage guard — never degraded to a bare "tests pass" — the coverage term is **specialized** (not overridden; the standard route's `AC→AT coverage` is unchanged): its covered target becomes the **cause-agreement-gate-approved failing reproduction test**. The coverage gate verifies, in a context separate from the AT author, that **the reproduction test exists and went 赤→緑** (the oracle anchor is real and the fix turned it green). This preserves AL-2/AL-3's external-anchor coverage guard (the loop never grades its own AT). The AND structure is objective-only (#355: no LLM-reviewer term): `AND(回帰テスト green [deterministic], 失敗再現テスト被覆 green — repro 赤→緑 confirmed externally)`.
...
### AL-4 — Objective evidence + auto-demote false-green
Every oracle term is evidence-backed by an objective signal: a test-command exit code (atGreen), a `check_red_evidence` result reading the red.jsonl record (redObserved, #355 F5), or the AC→AT coverage check (coverageOk). A claimed PASS with no backing objective evidence — atGreen asserted without an exit code, redObserved without a red.jsonl record (test_sha + impl_sha) — is **auto-demoted to FAIL and re-run** (fail-closed; the gates default false when AT is required). Every iteration's objective-gate state (`{redObserved, atGreen, coverageOk, uncovered}`) is fingerprinted and appended to `docs/issues/<NNN>-<slug>/autopilot-log.jsonl` as the external source of truth and audit trail. Carried fix findings (`rejectionFindings` / `implSeedFindings` / re-loop gate failures) still carry an `evidence_ref` (a failing-AT name / log path, an uncovered-AC quote, or a human-comment URL).

### AL-5 — Fail safe
autopilot halts and escalates to a human on non-convergence, budget overrun, or repeated identical failure. Mechanisms: `MAX_ITERATIONS` per step, a **sameness-detector** (normalized sha256 fingerprint identical twice in a row among **same-step FAIL rows only**), **stuck detection** (no progress across a window of 3 using **same-step FAIL rows only** — PASS rows are never part of the comparison population, #277), **gate-unverifiable early escalation** (#355 F6: when the AT pass and AC coverage holds but no red evidence can be read — `atGreen && coverageOk && !redObserved`, so the objective gate cannot be established; a *mechanism* failure, not an incomplete deliverable — autopilot does not exhaust MAX_ITERATIONS; it records `gate-unverifiable` via `record_halt` and immediately hands to a human), and `COMPLETED_WITH_DEBT` (record unresolved findings and hand to a human). Silent infinite loops and silent fake-green are structurally impossible.
...
autopilot does not fork or rewrite the flow skills. Each flow skill (`defining-requirements` … `running-atdd-cycle`) keeps its normal behavior. What changes **only while autopilot runs** is *where the User gate sits*: outside autopilot a human reviews each step; under autopilot the User gates collapse to requirements approval (start), design approval (before ATDD), and merge (end), and the steps between gates are looped autonomously against objective gates. This is why autopilot is implemented as a **thin orchestrator** (`autopilot`) over the existing skills rather than as edits to them — the skills' role is mode-dependent, their code is not. (#355: `reviewing-deliverables` is **not** part of the autopilot convergence loop — it remains a standalone, human-invoked review skill (Step 5); convergence is driven by objective gates and review judgment enters at the human merge gate.)
...
| ② 設計承認 | 人間がサインオフ | **生成された設計成果物を自動受理**（#355: design phase に客観ゲートも LLM レビューループも無く生成 1 回。客観オラクルは impl phase の red→green AT＋AC カバレッジ＝AL-3/AL-4 が担う）。人間は Draft PR の設計成果物を見て差し戻す **override 権を保持** |
| ③ merge | 人間が merge | **merge coordinator に委譲**（容量1直列・rebase 後フル再ゲート。autopilot は元々 merge しない） |

**正当性**: hand-off は「人間が境界を所有するか」を緩めない。①は queue キュレーション、②は設計成果物の自動受理＋人間の Draft PR override（#355: 設計に客観ゲートは無く、客観オラクルは impl phase で発火）、③は coordinator の再ゲート＋AL-5 のエスカレーション（N 回失敗で human フラグ）で担保される。緩めるのは「人間がループ途中で何回サインオフするか」だけで、これは通常 autopilot が②を人間に残すのに対し full-autopilot がそれを自動受理＋override に委ねる差にすぎない。**この上書きは hand-off フラグが立っている起動に厳密に閉じ、通常 autopilot の AL-1 を変更しない。**
