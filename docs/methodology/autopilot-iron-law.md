...
On the flaky route the standard AND oracle's `AC→AT coverage` term has **no approved AC set** to cover (no US/plan/AT spec). To keep the coverage guard — never degraded to a bare "tests pass" — the coverage term is **specialized** (not overridden; the standard route's `AC→AT coverage` is unchanged): its covered target becomes the **cause-agreement-gate-approved repeatedly-failing anchor**. The coverage gate verifies that **the repeatedly-failing anchor exists and went from red (observed at least once in N iterations) to N consecutive greens (determinization confirmed)** (the oracle anchor is real and the fix turned it deterministically green). This preserves AL-2/AL-3's external-anchor coverage guard (the loop never grades its own AT). The standard AL-3 AND structure stays four-term: `AND(反復 green [deterministic, N consecutive], 反復 failing アンカー被覆 green — 反復赤→N 回連続緑 confirmed externally, reviewer overall_correctness = correct, confirmed P0/P1 = 0)`.
