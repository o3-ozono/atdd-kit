...
@test "AT-372-1: 3-way split section heading exists" {
  grep -q '3分割' docs/methodology/a11y-test-taxonomy.md
}

@test "AT-372-1: all 3 category names are present" {
  grep -q '自動・静的' docs/methodology/a11y-test-taxonomy.md
  grep -q '自動・操作' docs/methodology/a11y-test-taxonomy.md
  grep -q '手動' docs/methodology/a11y-test-taxonomy.md
}
...
@test "AT-372-2: auto-green-not-equal section heading exists" {
  grep -q '自動 green' docs/methodology/a11y-test-taxonomy.md
}
...
@test "AT-372-3: applicability-criteria section has all required terms" {
  grep -q '適用基準' docs/methodology/a11y-test-taxonomy.md
...
@test "AT-372-4: separate-axis section and triage term are present" {
  grep -q '別軸' docs/methodology/a11y-test-taxonomy.md
  grep -q 'トリアージ' docs/methodology/a11y-test-taxonomy.md
}
