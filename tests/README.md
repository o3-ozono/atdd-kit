...
| test_running_atdd_cycle_skill.bats | running-atdd-cycle skill (Step 4, ATDD double loop) — #355 F5: impl baseline SHA as 4th arg to `record_red_evidence` in C2 Confirm RED; #355 F4: AT modality generalized (executable `tests/acceptance/` or skill/doc BATS pins under `tests/*.bats`), red-first applies to both modalities |
...
| Test File | Target |
|-----------|--------|
| test_autopilot_skill.bats | autopilot skill (autopilot orchestrator) — oracle / rails / three User gates (requirements, design approval, merge) / iron-law wiring / args contract (#256: fail-closed phase guard, no impl→design fallback) / dialog economy (#254: human-only questions, batch-present) / log-integrity plumbing (#262: logLines baseline + recorded counter + 'log-integrity' halt) / design-gate rejection plumbing (#261: `rejectionFindings` validation + iteration-1 seed, whole-set rejection discipline) / model assignment (#259: impl subagents Sonnet, one-way escalation, design phase excluded) / presentation channel (#267: Gate ①/② deliverable bodies as the Draft PR diff, terminal = PR link + decision points only, full-channel sync kept) / step-scoped sameness+stuck + oracle-state fingerprint (#272: AT-005 rails passes ${step} to check_sameness/check_stuck, AT-006 audit payload = JSON.stringify({ redObserved, atGreen, coverageOk, uncovered }) — #355: review-derived `blocking` removed) / diff-in-body (#275: boundary canary AT-000 guards the sed section headings against silent EOF expansion, re-presentation per-finding diff hunks + key lines, first-presentation key decisions with file/line refs, hand-off per-file stat + key hunks, operational definitions pinned, #267/#275 reconciliation pinned in both sections, ban polarity anchored, empty rejectionFindings refused fail-closed) / agent() null フェイルセーフ (#292: AT-001 at-gate atGreen null guard / AT-002 coverage cov null / AT-003 (#355: review verdict removed from the loop — pins the absence of the verdict term in `converged`) / AT-004 freeze frozen null / AT-005 audit rec null / AT-006 rails r null / AT-007 fail-open 禁止コメント) / design-gate AskUserQuestion (#305: split-doc existence + reference, line-budget no-3rd-raise, AT-001 Recommended-approval-first, AT-002 US/Plan/AT send-backs, AT-003 harness-auto Other, AT-004 unchanged approval/rejection semantics, AT-005 'recommended ok' fallback line, Gate ① delegates to defining-requirements with no separate requirements gate) / impl phase model assignment (#311: AT-001 MODEL const Sonnet default, AT-002 8 impl agent() all carry model: MODEL, AT-003 freeze:anchor excluded, AT-004 line-budget ≤ 280 maintained) / express precheck + route-eligibility.md loader split (#304: AT-301 express precheck section exists before User gates, AT-302 references route-eligibility.md as signal source, AT-303 presents advisory once + requires explicit ok, AT-401 auto-route forbidden, AT-402 precheck is pre-flight advisory not a gate, AT-403 route-eligibility.md has all 4 required elements) / halt terminating record wiring (#299: AT-002 all 8 impl agent labels carry model: MODEL; total count is exactly 8 — gen / red-gate / at-gate / coverage / audit / rails / audit-halt×2 (#355: in-loop `review` agent removed, gate-unverifiable audit-halt added); log-only commit; convergence-failure-only enum guard; recorded counter not incremented by HALT row) / red gate + objective satisfaction oracle (#334 / #355: AT-334-oracle oracle is 3-term objective `AND(redObserved, atGreen, coverageOk)` — the LLM-reviewer terms (`overall_correctness`, P0/P1) were removed (#355 F1); AT-334-gate red gate symmetric to AL-3 green gate, reads SHAs from red.jsonl record (#355 F5); default false fail-closed) / convergence redesign (#355: F1 objective oracle, F2 no in-loop reviewing-deliverables call, F3-1/F3-2 design no-review-loop + in-loop-reviewer framing removed, F6 gate-unverifiable = objective gate unestablishable, F7 reviewing-deliverables reverted to standalone — 119 tests total) |
| test_autopilot_convergence.bats | lib/autopilot_convergence.sh safety rails (behavioral: fingerprint / JSONL audit / sameness / stuck / max-iterations / log-integrity (#262: exact line-count match, fail-closed)) / step-scoped sameness+stuck (#272: AT-001 cross-step false-halt fix, AT-002 same-step halt preserved, AT-003 backward compat, AT-004a/b/c check_stuck step filter) / FAIL-only population (#277: AT-001 false-stuck-halt repro, AT-002 PASS-row exclusion from sameness, AT-003 legacy-mode FAIL-only, AT-004 detection-power maintained, AT-006 cross-run FAIL adjacency semantics pin) / record_halt + timestamp (#299: record_halt appends terminating HALT row; findings_digest as nested JSON array; timestamp field in record_iteration; timestamp not in fingerprint path; range-outside reason rejected) / red evidence recording + check (#334: record_red_evidence appends red exit to red.jsonl fail-closed; check_red_evidence requires test commit to precede impl commit in ancestry and red evidence present; empty/malformed inputs refused; commit sha with newline/double-quote refused) / convergence redesign (#355: F5 `record_red_evidence` impl_sha 4th arg + backward-compat + `check_red_evidence` records-value判定 + fail-closed; F6 `record_halt` gate-unverifiable enum accepted, unknown reason still rejected — 87 tests total) |
...
| test_impact_map_platform.bats | scripts/impact_map.sh platform adapter generalization — --platform {other\|web\|ios} routing / select_other non-breaking preservation / web src glob / ios Sources glob / unmatched fallback (Issue #323, 16 tests) |
...
| AT-308.bats | bugfix 専用の軽量ルート（フル機能ルートと分離）— route-eligibility.md bugfix 判定信号 + No Auto-Routing 維持・bugfix コマンド配線・bugfix 収束オラクル（回帰 green + 既存回帰なし + 再現テスト赤→緑）・AL-3 coverage 項の失敗再現テスト被覆 specialize・cause-agreement 安定トークンの iron-law / design-gate 二文書整合・half-scope + flaky フォローアップ #322・version/CHANGELOG 不変条件。wiring pin（赤→緑 ランタイム fix loop は own せず out-of-band replay 経路が own）（Issue #308、14 tests） |
...
| AT-348.bats | retrospective.sh の Dialogue Volume ゼロ化＋friction 分類異常修正 — munged-path が実 Claude Code 命名規則（先頭ダッシュ保持＋`/`・`.` を `-` へ）を解決し turns を正しく計数（AT-348-1、ドット含むパスで両不具合を exercise）・friction 分類で `extracting-user-stories`→design（AT-348-2）・`running-atdd-cycle`→新設 impl バケット（AT-348-3）・merge-family→merge 非回帰（AT-348-4）（Issue #348、4 tests） |
| AT-351.bats | bugfix コマンド配線確立 — `commands/bugfix.md` 存在＋fixing-bugs 配線＋旧コマンドファイル不在（AT-351-1）・live docs（commands/README.md / skills/README.md / skill-gate/SKILL.md / fixing-bugs/SKILL.md / route-eligibility.md / tests/README.md）が bugfix 表記へ完全移行（AT-351-2）・AT-308.bats パス pin と配線 grep が新コマンド名へ更新（AT-351-3）・fixing-bugs ルートのフロー・ゲート・オラクル不変（AT-351-4）・plugin.json version が CHANGELOG 最新リリース見出しと一致・#351 エントリ追記（AT-351-5）（Issue #351、21 tests） |
| AT-341-skill-contract.bats | batch-discovery 独立スキル契約 — SKILL.md の frontmatter（name: batch-discovery）/ 明示 trigger / Input（Issue 群 / 並列度 K）/ Responsibility Boundary（full-autopilot 非改変 C3）（AT-341-A1、8 tests） |
| AT-341-dispatch-reuse.bats | 横断バッチ壁打ち＋並列 dispatch — SKILL.md の自律ドラフト記述 / 人間判断 5 種 / 1 人間セッション束ね / 定数ゲート（AT-341-B1/B2）・lib/full-autopilot-dispatch.sh select による K スロット lease + busy Issue スキップ（AT-341-C1）・worktree 隔離＋播種必須＋Unknown-command 即死警告（AT-341-C2）・lease 3 経路 release（AT-341-C3）（Issue #341、18 tests） |
| AT-341-ready-to-go-gate.bats | 実装順序 manifest 軽量制御＋選別最終承認ゲート — manifest 共有真実源 / dispatcher 読み取り / barrier 非採用 Non-Goals（AT-341-D1）・覆りうる点あり→承認ゲート / ゼロ→スキップ / 一括承認禁止 / 0 or 1 セッション上限（AT-341-E1）・AL-1 Gate①/②/③ 対応表 / Gate③ merge = full-autopilot 不変 / AC 承認ゲート撤廃せず（AT-341-E2）（Issue #341、10 tests） |
| AT-341-gate-invariant.bats | AL-1 三ゲート不変条件と SKILL.md 整合の deterministic 検証 — Gate①=横断バッチ壁打ち / Gate②相当=選別最終承認(max 1) / Gate③=full-autopilot merge 不変 / false-green 外部アンカー非撤廃 / autopilot-iron-law.md AL-1 節存在・三ゲート固定・merge coordinator 記述 / skills/README.md + CHANGELOG #341 記載 / version 不変条件（AT-341-F1、11 tests） |
...
| test_ios_impact_rules.bats | iOS addon impact_rules.yml テンプレート構造 ＋ addon.yml deploy エントリ — impact_map.sh entry / impact_rules.yml entry / Sources/ 構造 / Tests/ 構造 / パーサ解釈（Issue #323、10 tests） |

## Web Addon Tests (addons/web/tests/)

| Test File | Target |
|-----------|--------|
| test_web_addon.bats | Web addon deploy entries ＋ impact_rules.yml テンプレート構造 — addon.yml 必須フィールド（name/display_name/deploy/detect）/ impact_map.sh・impact_rules.yml エントリ / package.json detect / src/ ・ tests/ パスルール / パーサ解釈（Issue #323、14 tests） |
