...
- `lib/transition_detector.sh`: stream-json multi-msg-per-turn handling. New `aggregated_tool_uses` field collects tool_use names across all assistant messages in the current response turn, bounded by the next real user input (skill-body injection user-text msgs starting with `Base directory for this skill:` are correctly classified as transparent runtime injections, not turn boundaries). `same_turn_spawn` now evaluates against the aggregated count, fixing false negatives where headless `claude -p` produces thinking + text + tool_use across separate assistant messages instead of a single combined message. (#162)
- `tests/claude-code/samples/integration-autopilot-phase1-transition.sh`: ATDD-correct outer test infrastructure. Adds `SKILL_TEST_PLUGIN_DIR` env var to swap the plugin context (used to drive RED state via a pre-fix worktree); runs `claude -p` from an isolated cwd to prevent fix-state leakage from worktree files (test fixtures, lib/, .git history); injects `commands/autopilot.md` from `$SKILL_TEST_PLUGIN_DIR` via `--append-system-prompt-file` so the orchestrator's dispatch awareness tracks the plugin under test; rewrites the user prompt to be self-contained (no file-path references, no explicit same-turn instruction, no NEXT_REQUIRED_ACTION mention) so the dispatch metadata in autopilot.md is the actual driver of behavior. The assertion `agent_count == 2` discriminates both regression directions: under-spawn (the original #162 turn-crossing bug) and over-spawn (pre-fix imperative text running past Phase 1 into TeamCreate/Phase 2+). Verified RED→GREEN: pre-fix plugin → 35 tool_uses including 4 Agent over-spawn (FAIL); fix plugin → exact 2 Agent same-turn spawn then stop (PASS). (#162)
