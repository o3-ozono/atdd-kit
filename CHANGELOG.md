...
### Added
- `docs/methodology/scrumban.md`: `## GitHub Project` section — live Project URL, 7-field schema (6 custom fields + Iteration), Status↔autopilot label mapping table with intentional gap note for "Shaped (Pitch済)". (#168)
- `scripts/setup-project.sh`: idempotent CLI script for GitHub Projects v2 setup — project create guard, Status + 5 custom fields creation, all Open Issue bulk-add, bulk field-set (uses `--single-select-option-id` and GraphQL node ID for `--project-id`). (#168)
...
- `tests/test_github_projects_setup.bats`: 22-assertion BATS test suite covering AC1–AC5 for the setup scripts and scrumban.md GitHub Project section. (#168)
