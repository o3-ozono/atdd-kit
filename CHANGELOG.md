...
- **`docs/methodology/prioritization.md` を新設（#367）**。MoSCoW（DSDM）を一次情報として扱い、atdd-kit の `[独自]` 拡張（5 段階: MUST/SHOULD/WANT/次回以降/破棄、効き×工数の 2 軸判定、anti-pattern、破棄要件の理由付き保存によるゾンビ復活防止）を明文化。`defining-requirements` スキルの Section 4 — 機能要件 から本 doc を参照する接続を追加。`tests/test_prioritization_doc.bats` で必須セクションを構造検証。
...
- **session-start の「別セッション作業中」検出を branch-lease store ベースにする（#343）**。
...
- **フロー完了時の自動振り返り（メトリクス + フィードバック抽出）（#309）**。
  - `scripts/retrospective.sh` を新規作成。引数: `--issue <N> [--pr <PR>] [--dry-run] [--json-output] [--help]`。ゼロ LLM 呼び出し・ゼロ対話入力・5 秒以内完了（CS-1 軽量性）。一次ソース: headless worker 出力 JSON（`usage`/`total_cost_usd`）・セッション transcript（ターン数）・`autopilot-log.jsonl`（フェーズ境界のみ、トークン不使用）・`gh` コメント（摩擦点）。自動起票なし（No Auto-Routing）。横断 JSONL `docs/retrospective-log.jsonl` に append-only 追記。
  - `templates/docs/issues/retrospective.md` を新規作成（5 区分テンプレート: 対話量 / コスト / 前回比 / 摩擦点 / 改善候補）。
...
- **full-autopilot の成否判定が「空成果」を merge-ready 誤判定する穴を塞ぐ（#333）**。`lib/full-autopilot-run.sh` の `__default_result()` に、merge-ready ラベル照合の前段で fail-closed ガードを追加: worker の出力 JSON が `num_turns:0`（worker が実装に未到達）または `result` に `Unknown command`（プラグイン/コマンド未解決）を含む場合は、`is_error:false` の自己申告や stale な `merge-ready` ラベルの有無に関わらず `failed` を返す（#329 H6「成否判定の自己申告依存」の縦深防御）。
  ...
  - レビュー指摘（minor/nit）を反映: パス参照を `$f` に統一（DRY）、`num_turns` 正規表現をコロン後スペース許容（compact/pretty 両対応）に強化。
  - `tests/acceptance/AT-333-empty-guard.bats`（a/b/c: 空成果/Unknown command ＋ merge-ready ラベル時 `failed` / d/e: 実成果・`num_turns:20` 部分一致ガードの happy-path 回帰防止 / f: out.json 不在の fail-closed / g: 整形済み JSON の `num_turns: 0` 検知、7 tests）。
...
- **full-autopilot の使い勝手再設計（真因0-4 一括）（#329）**。
  ...
  - **真因0 follow-up — headless worker の worktree にプラグイン設定を播種**。`lib/full-autopilot-run.sh` の `__default_launch` を worktree 隔離対応に拡張（既定 `<repo の親>/<repo 名>-<issue>`、`FA_WORKTREE_DIR` 上書き / `FA_NO_WORKTREE=1` で cwd 起動）。`.claude/settings.local.json`（`enabledPlugins` + `extraKnownMarketplaces`）は gitignore 対象で `git worktree add` では複製されず、播種しないと headless `claude -p` worker が `Unknown command: /atdd-kit:autopilot` で即死する。新ヘルパ `__seed_worktree_settings`（冪等: source 不在 no-op / 既存一致 skip）を起動前に必ず呼ぶ。`skills/full-autopilot/SKILL.md` flow step 3 に明文化。`tests/acceptance/AT-329-worktree.bats`（wt-a〜e、5 tests）。
