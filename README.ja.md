...
**Autopilot モード**がハンドルを握ります。main Claude がオーケストレーターとしてタスクタイプ別チーム — Developer、QA、Tester、Reviewer、Researcher、Writer — を統括し、Issue からマージまで一気通貫。重要なチェックポイントはあなたが承認し、残りはエージェントが処理します。
...
| コマンド | 概要 |
|---------|------|
| `/atdd-kit:autopilot` | Autopilot — main Claude がオーケストレーターとしてタスクタイプ別 Agent Teams で Issue を end-to-end 完遂 |
...
```mermaid
flowchart TD
  P1["Phase 1: discover\n(main Claude 主導)"] --> P2["Phase 2: plan\n(main Claude 統括、タスクタイプ別エージェント主導)"]
  P2 --> P3{"タスクタイプ分岐"}
...
main Claude がオーケストレーター（PO 役割）としてタスクタイプ別チームを統括します。6つのエージェントが利用可能:
...
| research | main Claude | main Claude, Researcher x N（テーマ毎に最低2名） |
...
`/atdd-kit:autopilot` は Agent tool でサブエージェント（developer / qa / tester / reviewer / researcher / writer）を spawn します。`.claude/config.yml` を編集すれば、ロールごとのモデルを固定できます:
...
モデル列挙: `sonnet`, `opus`, `haiku`。リストに無いロールは **session default** を継承します（Agent tool の `model` パラメータが省略されます）。

### 起動パス

| コマンド | 挙動 |
|---|---|
| `/atdd-kit:autopilot 123` | `spawn_profiles.custom` があれば自動適用、無ければ全ロール session default。確認ゲート無し。 |
| `/atdd-kit:autopilot 123 --profile="reviewer only heavy"` | `custom` を base に NL を重ねる — 重複時は NL 優先、どちらにも無いロールは session default。Team / worktree 作成前に Profile Confirmation Gate で resolved matrix を確認。 |

Main Claude（orchestrator）は常に session default を保持します。プロファイルはサブエージェント spawn のみに影響します。

### BREAKING 注記 (Issue #122)

従来の `--light` / `--heavy` preset フラグは **廃止** されました。指定すると
`Unknown flag: --light (removed in BREAKING change; use --profile="..." instead. supported: --profile)` （`--heavy` 時は置換）で halt します。代替として `.claude/config.yml` の `spawn_profiles.custom` で既定を定義するか、`--profile="..."` で一時上書きしてください。

従来の `.claude/workflow-config.yml` と plugin 側 `config/spawn-profiles.yml` は `.claude/config.yml` に統合されました。次回セッションで `session-start` が自動移行します。
