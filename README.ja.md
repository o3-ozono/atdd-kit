...
**6 ステップのフロー**が各 Issue を要件からデプロイまで運びます: 要件定義 → ユーザーストーリー抽出 → 計画 + 受け入れテスト作成 → ATDD サイクル → レビュー → マージ & デプロイ。各ステップは直接呼び出すスキルで、レビューステップでは動的レンズパネルを並列実行してマージ前にすべての成果物を検査します。
...
| 5 | **reviewing-deliverables** | 全成果物（PRD/US/Plan/Code/AT）を動的レンズパネル × 並列 Workflow（#234）でレビューし、単一の PASS/FAIL を出力 |
...
| **autopilot** | autopilot — flow skill をループして Issue の成果物を near-green まで自律収束。User gate は要求承認（最初）・設計承認（ATDD 前）・merge（最後）の3点 |
...
レビューステップ（`reviewing-deliverables`、Step 5）は動的レンズパネル × 並列 Workflow（#234）を使用します:

1. **Scout** — この Issue でスコープに含まれる成果物を特定する
2. **Generate** — 成果物の内容に合わせた動的レビューパネルを構築する
3. **Review** — レンズを並列実行し、各レンズが 1 つの成果物を受け入れ基準に照らして検査する
4. **Verify** — 知見の敵対的クロスチェック
5. **Aggregate** — 各レンズ結果へのトレーサビリティを保った単一の PASS/FAIL を出力する
