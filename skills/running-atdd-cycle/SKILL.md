...
- **C1 — Concrete Examples.** Each Acceptance Criterion is expressed as a Concrete Example in domain language: `Given <precondition>`, `When <action>`, `Then <expected>`. The AT file is the executable form of that example. **The AT modality follows what the change is (#355 F4):** behavior changes use executable tests under `tests/acceptance/`; **skill / doc / shell changes** (where the deliverable is SKILL.md / docs / lib content) use **BATS pins under `tests/*.bats`** asserting the changed content. Either modality is a first-class AT — the plan declares which one applies.
...
- **C4 — Story unit vs unit unit.** Each Acceptance Test is scoped to **one User Story** — story-scoped, whether it lives at `tests/acceptance/AT-<NNN>.*` (behavior) or as a BATS pin block in `tests/*.bats` (skill/doc, #355 F4). Each TDD test is scoped to **one unit**. The two granularities never collapse into one.
...
1. **Write the AT (C1).** Express the AC as a Concrete Example (`Given` / `When` / `Then`) and encode it in the modality the plan declares (#355 F4): an executable test at `tests/acceptance/AT-<NNN>.*` for behavior, or a BATS pin block in `tests/*.bats` asserting the changed SKILL.md / docs / lib content for skill/doc changes. Mark it `[draft]` in `acceptance-tests.md`.
2. **Confirm RED (C2).** Run the AT and confirm it **fails (RED)**. This applies to BOTH modalities (#355 F4): a BATS pin for a skill/doc change must fail BEFORE the SKILL.md / docs / lib edit (the asserted content does not yet exist) and pass after — exactly like an executable AT goes red→green around the impl. Never proceed past a test that has not been observed failing. **Commit the AT file now** (test commit) before writing any implementation — this commit separation is mandatory (#334): the test commit precedes the impl commit in history, providing machine-verifiable red→green evidence. **Call `record_red_evidence <red.jsonl> <test-commit-sha> <at-file> <impl-baseline-sha>`** immediately after the test commit to record the red exit in `red.jsonl`; `<impl-baseline-sha>` is the current HEAD at red-observation time (before any impl commits) — pass it as the 4th argument so the impl_sha is recorded alongside the test SHA (#355 F5); without this call no evidence is written and `check_red_evidence` will fail-closed (deterministic, no LLM self-assessment).
