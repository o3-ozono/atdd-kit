...
2. **Confirm RED (C2).** Run the AT and confirm it **fails (RED)**. Never proceed past a test that has not been observed failing. **Commit the AT file now** (test commit) before writing any implementation — this commit separation is mandatory (#334): the test commit precedes the impl commit in history, providing machine-verifiable red→green evidence via `record_red_evidence` + `check_red_evidence` (deterministic, no LLM self-assessment).
