...
3. **Design-approval gate (human).** Present the near-green `user-stories.md` / `plan.md` / `acceptance-tests.md` and ask:
   ...
   **Diff-in-body (mandatory, #275).** The gate message itself must carry the evidence, inline in BOTH the in-session message and the GitHub gate comment: on every re-presentation after fixes, show the actual diff hunks of what changed (```diff blocks organized per finding, with the load-bearing lines called out); on first presentation, show each artifact's key decisions with file/line references. Never present a summary-only gate that makes the human ask for the diff.
...
5. **Hand off.** The near-green Issue goes to the human merge gate (`merging-and-deploying`). Diff-in-body (#275) applies here too: the hand-off message includes the implementation diff inline (per-file stat + the key hunks), not just a green-status summary.
