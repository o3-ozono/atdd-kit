...
1. **Precondition.** The approved PRD exists at `docs/issues/<NNN>-*/prd.md`. Missing or unapproved → stop and route to `defining-requirements`. **Label pre-flight check (#370).** Before proceeding, run `scripts/check-required-labels.sh` as a startup guard: it checks the workflow labels (`in-progress` / `ready-to-go` / etc., canonical source `commands/setup-github.md`) required by this skill's label transitions. Per `docs/design/setup-on-demand-policy.md`, missing labels are **notified only — never error-exit** and the check is **skippable** (gh absent/unauthenticated → skip gracefully); this guard never blocks autopilot from proceeding.
