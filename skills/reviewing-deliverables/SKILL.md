...
1. **Scout** — one agent reads the deliverables and reports their types, languages, change size, and every **risk surface** the change touches (auth, input/IO, network, concurrency, data/migration, UI/UX, performance-sensitive paths, error handling). **Scope guard (#355 F5):** extract the Issue boundary from the PRD/US; tag findings against out-of-scope files or concerns as `out-of-scope` — these are not FAIL factors, but follow-up candidates.
...
4. **Verify (multi-round, adversarial)** — each finding is independently challenged by several skeptics across diverse angles; a finding survives only on **majority** confirmation. This suppresses false positives. **Round memory (#355 F6):** findings where the implementer declared a deliberate trade-off via a docstring or ADR are not re-raised across rounds unless the panel votes them unjustified by majority. **Severity dedup (#355 F7):** identical findings from multiple lenses are merged cross-lens into one entry before severity is assigned — severity is assigned once per merged finding, eliminating duplicate severity assignment.
5. **Aggregate** — one agent consolidates the surviving findings and per-lens notes into a single **PASS / FAIL** verdict, written in Japanese. **Aggregate rules (#355 F3):** panel covers N=3 mandatory stances — functional-correctness, safety (adversarial skeptic), design-validity (advocate) — plus dynamic risk lenses. A blocker/major finding is adopted only when 2/3 or more of relevant lenses confirm it; a solo single-lens finding has its severity downgraded one level. **Convergence / stop condition (#355 F4):** after fixes the loop reaches **CONVERGED** (conditional PASS) when (a) zero new blocker/major findings, or (b) all remaining findings are tagged `design-judgment` or `out-of-scope`. Maximum round limit: 3 rounds per phase to prevent infinite FAIL loops. It additionally emits a backward-compatible machine-readable verdict (`overall_correctness` plus `findings[]` carrying `priority` / `confidence` / `evidence_ref`) that `autopilot` (#246) consumes to drive its satisfaction-oracle loop; non-autopilot callers ignore the extra fields.
