...
2. **Generate** — one agent turns the scout report into a **dynamic** review panel. It ALWAYS includes functional-correctness, clean-code, testability, and a positive **advocate** + a negative **skeptic**; it ADDS a non-functional lens (security, performance / load, usability, …) **only for each risk surface the scout actually found**. Panel size and personas therefore scale with the change.
