...
- **キュー** = `ready-to-go` ラベルの open Issue 群（`gh issue list --label ready-to-go`）。**`ready-to-go` = DoR（`docs/methodology/definition-of-ready.md`）＋ plan review PASS が前提。** DoR 未達 / plan 未承認の Issue は queue に乗らない＝着手しない（暴走防止の安全弁 / C4）。
- 並列度 K（`--parallel`、既定 2）。

## Flow

1. **キュー取得.** `ready-to-go` の Issue 番号を優先度順（古い順 / 明示優先度ラベル）に列挙する。**ラベル pre-flight check（#370）**: キュー取得前に `scripts/check-required-labels.sh` を起動時ガードとして呼び出す。`docs/design/setup-on-demand-policy.md` の標準パターンに従い、不足は通知のみ・エラー終了しない・skip 可（gh 不在/未認証は skip）。各 worker（`autopilot --hand-off`）側でも同じ pre-flight check が Precondition ステップで走る（二重チェックではなく起動経路ごとの独立ガード）。
...
3. **worker 起動.** 選ばれた各 Issue について、独立 worktree を作り **プラグイン設定を播種してから** **headless プロセス**として起動する:
   ```
   ...
   - **worktree へのプラグイン設定播種（#329）**。`.claude/settings.local.json`（`enabledPlugins` + `extraKnownMarketplaces`）は gitignore 対象で `git worktree add` では複製されない。播種せず headless worker を起動すると atdd-kit プラグインを読み込めず `Unknown command: /atdd-kit:autopilot` で即死する。launcher（`__default_launch`）は起動前に必ずメインチェックアウトの設定を worktree へ播種する（冪等）。worktree 既定パスは `<repo の親>/<repo 名>-<issue>`（`FA_WORKTREE_DIR` で上書き、`FA_NO_WORKTREE=1` で cwd 起動）。
...
4. **完了監視.** `run_in_background` は同期 stdout を返さずハンドルのみ返す。**完了通知を受けてから**リダイレクト先 `<worker-out>.json` を読み（`is_error` / `terminal_reason` / `total_cost_usd`）結果を判定する（既定の result 判定は `is_error:false`＝成功。`merge-ready` ラベルでの二重確認が要るなら `FA_RESULT_CMD` を差し替える。実行中のポーリングが要るなら Monitor / 共有ファイル）。ログは3層回収（stdout json / session transcript / 入れ子 Workflow の `subagents/workflows/wf_*/`）し監査ログへ集約。
